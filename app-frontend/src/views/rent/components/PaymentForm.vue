    ...
    <form class="form" @submit.prevent="submitPayment">
      <div class="credit-card-info--form">
        ...
        <div class="input_container">
          ...
          <div class="split">
            <input
              ...
              title="Expiry Date"
              type="text"
              class="input_field expiry_date_field"
              id="expiry_date"
              required
              maxlength="5"
            ...
            <input
              ...
              title="CVV"
              type="text"
              class="input_field cvv_input_field"
              id="cvv_field"
              required
              maxlength="3"
        ...
        <div class="save-card-option">
          <AnimatedCheckbox v-model="saveCardInfo" />
          <label class="save-card-label">Save card information for future payments</label>
        </div>
        ...
        <div class="payment--options">
          <button type="button" name="paypal" @click="handleWalletPayment('paypal')">
            <svg
          ...
          <button type="button" name="apple-pay" @click="handleWalletPayment('apple')">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              xmlns:xlink="http://www.w3.org/1999/xlink"
              version="1.1"
          ...
          <button type="button" name="google-pay" @click="handleWalletPayment('google')">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="80"
              height="39"
...
import { saveBankCard, getUserInfo } from '@/service/user'; // 导入用户服务方法
import AnimatedCheckbox from './AnimatedCheckbox.vue'; // 导入动画复选框组件


const emit = defineEmits(['payment-success', 'payment-error', 'payment-cancel']);

const userStore = useUserStore();
const saveCardInfo = ref(false); // 是否保存卡信息
...
const validateNumericInput = (event) => {
  ...
  cardData.value.cvv = event.target.value;
};
...
const submitPayment = async () => {
  try {
    ...
    // 构建支付数据对象 - 移除了holderName
    const paymentData = {
      type: 'card',
      cardNumber: cardData.value.cardNumber.replace(/\s/g, ''),
      expiryDate: cardData.value.expiryDate,
      cvv: cardData.value.cvv
    };
    
    // 如果用户选择保存卡信息，调用保存卡信息API
    if (saveCardInfo.value) {
      try {
        ...
        // 从过期日期中提取月份和年份
        const [month, year] = cardData.value.expiryDate.split('/');
        
        // 调用保存银行卡API
        const saveRes = await saveBankCard(
          account,
    ...
    emit('payment-error', error.message || '支付处理失败');
  }
};
...
const handleWalletPayment = (walletType) => {
  try {
    ...
    // 发送支付数据到父组件
    emit('payment-success', walletPaymentData);
  } catch (error) {
    console.error(`${walletType} payment error:`, error);
    emit('payment-error', error.message || `${walletType} payment failed`);
  }
};
...
const validateCardData = () => {
  ...
  if (cardNumberClean.length !== 16 || !/^\d+$/.test(cardNumberClean)) {
    emit('payment-error', 'Please enter a valid 16-digit card number');
    return false;
  }
  
  // 验证过期日期(应为MM/YY格式)
  if (!/^\d{2}\/\d{2}$/.test(cardData.value.expiryDate)) {
    emit('payment-error', 'Please enter a valid expiry date (MM/YY)');
    return false;
  }
  
  // 验证CVV(应为3位数字)
  if (!/^\d{3}$/.test(cardData.value.cvv)) {
    emit('payment-error', 'Please enter a valid 3-digit CVV');
    return false;
  }
...
<style scoped>
  ...
  /* Apple Pay图标调整 */
  .payment--options button[name="apple-pay"] svg {
    width: 60px;
  }
  
  /* Google Pay图标调整 */
  .payment--options button[name="google-pay"] svg {
    width: 60px;
  }
