<template>
	<a-card :bordered="false">
		<s-table ref="tableRef" :columns="columns" :data="loadDate" :expand-row-by-click="true" :showPagination="false"
			bordered>
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'fieldJavaType'">
					<a-select class="xn-wd" v-model:value="record.fieldJavaType" :options="fieldJavaTypeOptions" placeholder="请选择"
						:disabled="toCommonFieldEstimate(record)" @change="fieldJavaTypeChange(record)" />
				</template>
				<template v-if="column.dataIndex === 'effectType'">
					<a-select class="xn-wd" v-model:value="record.effectType" :options="effectTypeOptions" placeholder="请选择"
				...
				<template v-if="column.dataIndex === 'dictTypeCode'">
					<a-select
						...
						class="xn-wd" v-model:value="record.dictTypeCode" :options="dictTypeCodeOptions" placeholder="请选择字典" />
					<span v-else>无</span>
				</template>
				...
				<template v-if="column.dataIndex === 'whetherRequired'">
					<a-checkbox v-model:checked="record.whetherRequired"
				...
				<template v-if="column.dataIndex === 'queryType'">
					<a-select v-if="record.queryWhether === true && record.effectType !== 'datepicker'" class="xn-wd"
						v-model:value="record.queryType" :options="queryTypeOptions" placeholder="请选择" />
