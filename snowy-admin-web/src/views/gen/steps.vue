<template>
	<div>
		...
		<div v-if="current === 2">
			<a-card>
				<a-result status="success" title="Operation成功" sub-title="此刻可预览代码，同时您可以一键生成代码啦">
					<template #extra>
						<a-space size="middle">
							<a-button v-if="current > 0" class="xn-ml8" @click="genPreviewRef.onOpen(recordData)">预览</a-button>
							<a-button v-if="current === steps.length - 1" type="primary" :loading="submitLoading"
								@click="seveGenerate">生成并关闭</a-button>
						</a-space>
					</template>
				</a-result>
			</a-card>
