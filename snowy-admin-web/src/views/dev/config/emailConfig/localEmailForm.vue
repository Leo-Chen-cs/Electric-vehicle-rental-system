<template>
	<a-spin :spinning="loadSpinning">
		<a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical"
			:label-col="{ ...layout.labelCol, offset: 0 }" :wrapper-col="{ ...layout.wrapperCol, offset: 0 }">
			...
			<a-form-item label="SMTP服务端口：" name="SNOWY_EMAIL_LOCAL_SMTP_PORT">
				<a-input-number v-model:value="formData.SNOWY_EMAIL_LOCAL_SMTP_PORT" placeholder="请输入SMTP服务端口"
					style="width: 100%" />
			</a-form-item>
			...
			<a-form-item label="是否使用STARTTLS安全连接：" name="SNOWY_EMAIL_LOCAL_STARTTLS_ENABLE">
				<a-switch v-model:checked="formData.SNOWY_EMAIL_LOCAL_STARTTLS_ENABLE" placeholder="请选择是否使用STARTTLS安全连接" />
			</a-form-item>
			<a-form-item>
				<a-button type="primary" :loading="submitLoading" @click="onSubmit()">保存</a-button>
				<a-button class="xn-ml10" @click="() => formRef.resetFields()">Reset</a-button>
			</a-form-item>
		</a-form>
	</a-spin>
</template>
