				...
				<a-form ref="formRef" name="advanced_search" :model="searchFormState" class="ant-advanced-search-form">
					<a-space>
						<a-radio-group v-model:value="opLogType" button-style="solid">
							<a-radio-button v-for="opLog in opLogTypeList" :key="opLog.value" :value="opLog.value"
								@click="opLogTypeClock(opLog.value)">
								{{ opLog.label }}
							</a-radio-button>
						</a-radio-group>
						<a-input-search v-model:value="searchFormState.searchKey" placeholder="请输入名称关键词" enter-button allowClear
							@search="onSearch" />
						<a-popconfirm title="确定清空Operation与异常日志吗？" @confirm="deleteBatchOpLog()">
