<template>
	<xn-form-container title="详情" :width="700" :visible="visible" :destroy-on-close="true" @close="onClose">
		<a-descriptions :column="1" size="middle" bordered class="mb-2">
			...
			<a-descriptions-item label="Operation类">{{ formData.className }}</a-descriptions-item>
			<a-descriptions-item label="Operation方法">{{ formData.methodName }}</a-descriptions-item>
			<a-descriptions-item label="Operation时间">{{ formData.opTime }}</a-descriptions-item>
			<a-descriptions-item label="Operation人">{{ formData.opUser }}</a-descriptions-item>
		</a-descriptions>
