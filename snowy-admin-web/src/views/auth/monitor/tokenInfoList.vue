<template>
	<xn-form-container title="令牌列表" :width="650" :visible="visible" :destroy-on-close="true" @close="onClose">
		<a-button danger class="xn-mb10" :disabled="selectedRowKeys.length === 0" :loading="beatchExitLoading"
			@click="beachExitTokenValue">批量强退</a-button>
		<a-table :columns="columns" :data-source="loadData" :row-selection="rowSelection" bordered
			:row-key="(record) => record.tokenValue" size="small">
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'tokenTimeout'">
					<a-tooltip>
						...
						<a-progress v-if="record.tokenTimeoutPercent * 100 > 80" :percent="record.tokenTimeoutPercent * 100"
							:show-info="false" status="success" />
						...
						<a-progress v-if="record.tokenTimeoutPercent * 100 < 20" :percent="record.tokenTimeoutPercent * 100"
							:show-info="false" status="exception" />
					</a-tooltip>
				</template>
