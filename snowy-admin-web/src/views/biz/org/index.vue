		...
		<a-col :xs="24" :sm="24" :md="24" :lg="19" :xl="19">
			...
			<a-card :bordered="false">
				<s-table ref="tableRef" :columns="columns" :data="loadData" :expand-row-by-click="true"
					:alert="options.alert.show" bordered :row-key="(record) => record.id" :tool-config="toolConfig"
					...
					<template #operator class="table-operator">
						<a-space>
							...
							<xn-batch-button v-if="hasPerm('bizOrgBatchDelete')" buttonName="Batch Delete" icon="DeleteOutlined"
								buttonDanger :selectedRowKeys="selectedRowKeys" @batchCallBack="deleteBatchOrg" />
						</a-space>
					</template>
					<template #bodyCell="{ column, record }">
						...
						<template v-if="column.dataIndex === 'action'">
							<a @click="formRef.onOpen(record)" v-if="hasPerm('bizOrgEdit')">Edit</a>
