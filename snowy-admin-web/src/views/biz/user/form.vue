<template>
	<xn-form-container :title="formData.id ? 'Edit人员' : '增加人员'" :width="800" :visible="visible" :destroy-on-close="true"
		:body-style="{ 'padding-top': '0px' }" @close="onClose">
				...
				<a-tab-pane key="1" tab="基础信息" force-render>
					...
					<a-row :gutter="16">
						<a-col :span="8">
							<a-form-item label="选择组织：" name="orgId">
								<a-tree-select v-model:value="formData.orgId" class="xn-wd"
									:dropdown-style="{ maxHeight: '400px', overflow: 'auto' }" placeholder="请选择组织" allow-clear
									tree-default-expand-all :tree-data="treeData" :tree-default-expanded-keys="treeDefaultExpandedKeys"
									:field-names="{
										children: 'children',
										label: 'name',
										value: 'id'
									}" @change="selePositionData(formData.orgId, 0)"></a-tree-select>
							</a-form-item>
						</a-col>
						<a-col :span="8">
							<a-form-item label="选择职位：" name="positionId">
								<xn-page-select ref="xnPositionPageSelectRef" v-model:value="formData.positionId" placeholder="请选择职位"
									allow-clear :page-function="selectApiFunction.positionSelector"
									:echo-function="selectApiFunction.echoPosition" />
							</a-form-item>
						</a-col>
						<a-col :span="8">
							<a-form-item label="选择主管：" name="directorId">
								<xn-page-select ref="xnUserPageSelectRef" v-model:value="formData.directorId" placeholder="请选择主管"
									allow-clear :page-function="selectApiFunction.userSelector"
					...
					<a-form-item label="任职信息" name="positionJson">
						...
						<div :key="positionInfo" v-for="(positionInfo, index) in formData.positionJson">
							<a-row :gutter="10">
								<a-col :span="7">
									<a-form-item :name="['positionJson', index, 'orgId']" :rules="{ required: true, message: '请选择机构' }">
										<a-tree-select v-model:value="positionInfo.orgId" class="xn-wd"
											:dropdown-style="{ maxHeight: '400px', overflow: 'auto' }" placeholder="请选择机构" allow-clear
											tree-default-expand-all :tree-data="treeData"
											:tree-default-expanded-keys="treeDefaultExpandedKeys"
											:field-names="{ children: 'children', label: 'name', value: 'id' }"
											@select="childOrgSelect(positionInfo, 0, index)"></a-tree-select>
									</a-form-item>
								</a-col>
								<a-col :span="7">
									<a-form-item :name="['positionJson', index, 'positionId']"
										:rules="{ required: true, message: '请选择岗位' }">
										<xn-page-select ref="xnChildPositionPageSelectRef" v-model:value="positionInfo.positionId"
											placeholder="请选择岗位" allow-clear :page-function="selectApiFunction.childPositionSelector"
											:echo-function="selectApiFunction.echoPosition" />
									</a-form-item>
								</a-col>
								<a-col :span="7">
									<a-form-item :name="['positionJson', index, 'directorId']">
										<xn-page-select ref="xnChildUserPageSelectRef" v-model:value="positionInfo.directorId"
											placeholder="请选择主管" allow-clear :page-function="selectApiFunction.childUserSelector"
											:echo-function="selectApiFunction.echoUser" />
									</a-form-item>
								</a-col>
					...
					<a-row :gutter="16">
						<a-col :span="12">
							<a-form-item label="家庭住址：" name="homeAddress">
								<a-textarea v-model:value="formData.homeAddress" placeholder="请输入家庭住址"
									:auto-size="{ minRows: 2, maxRows: 5 }" allow-clear />
							</a-form-item>
						</a-col>
						<a-col :span="12">
							<a-form-item label="通信地址：" name="mailingAddress">
								<a-textarea v-model:value="formData.mailingAddress" placeholder="请输入通信地址"
									:auto-size="{ minRows: 2, maxRows: 5 }" allow-clear />
							</a-form-item>
						</a-col>
					</a-row>
					...
					<a-row :gutter="16">
						<a-col :span="12">
							<a-form-item label="文化程度：" name="cultureLevel">
								<a-select v-model:value="formData.cultureLevel" placeholder="请选择文化程度" :options="cultureLevelOptions">
								</a-select>
							</a-form-item>
						</a-col>
						<a-col :span="12">
					...
					<a-row :gutter="16">
						<a-col :span="12">
							<a-form-item label="紧急联系人地址：" name="emergencyAddress">
								<a-textarea v-model:value="formData.emergencyAddress" placeholder="请输入紧急联系人地址"
									:auto-size="{ minRows: 2, maxRows: 5 }" allow-clear />
							</a-form-item>
						</a-col>
					</a-row>
