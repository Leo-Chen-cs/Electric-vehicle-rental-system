<template>
	<a-card :bordered="false">
		<a-space>
			...
			<a-input-search v-model:value="searchFormState.searchKey" placeholder="请输入菜单名称关键词" enter-button allowClear
				@search="onSearch" />
		</a-space>
	</a-card>
	<a-card :bordered="false" class="mt-2">
		<s-table ref="tableRef" :columns="columns" :data="loadData" :alert="options.alert.show" bordered
			:row-key="(record) => record.id" :show-pagination="false" :tool-config="toolConfig"
			:row-selection="options.rowSelection" :scroll="{ x: 'max-content' }">
			<template #operator class="table-operator">
				<a-space>
					...
					<xn-batch-button buttonName="Batch Delete" icon="DeleteOutlined" buttonDanger
						:selectedRowKeys="selectedRowKeys" @batchCallBack="deleteBatchMenu" />
				</a-space>
			</template>
			...
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						<a @click="formRef.onOpen(record, moduleType)">Edit</a>
