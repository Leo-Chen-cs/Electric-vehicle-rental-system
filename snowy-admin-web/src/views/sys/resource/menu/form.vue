<template>
	<xn-form-container :title="formData.id ? 'Edit菜单' : '增加菜单'" :width="700" :visible="visible" :destroy-on-close="true"
		@close="onClose">
			...
			<a-row :gutter="16">
				...
				<a-col :span="12">
					<a-form-item label="菜单类型：" name="menuType">
						<a-radio-group v-model:value="formData.menuType" button-style="solid" :options="categoryOptions"
							option-type="button" />
					</a-form-item>
				</a-col>
				<a-col :span="12">
					<a-form-item label="上级菜单：" name="parentId">
						<a-tree-select v-model:value="formData.parentId" v-model:treeExpandedKeys="defaultExpandedKeys"
							class="xn-wd" :dropdown-style="{ maxHeight: '400px', overflow: 'auto' }" placeholder="请选择上级菜单" allow-clear
							tree-default-expand-all :tree-data="treeData" :field-names="{
								children: 'children',
								label: 'title',
								value: 'id'
							}" selectable="false" tree-line @change="parentChange(formData.parentId)" />
					</a-form-item>
				</a-col>
				...
				<a-col :span="12" v-if="formData.menuType === 'MENU'">
					<a-form-item name="name">
						...
						<a-input v-model:value="formData.name" addon-before="setup name=" placeholder="请输入组件组件中name属性"
							allow-clear />
					</a-form-item>
				</a-col>
				<a-col :span="12">
...
const parentChange = (value) => {
	if (value > 0) {
		// 执行接口去Search选择的上级是哪个模块，吧对应的也置为一样的
		const param = {
			id: value
		}
