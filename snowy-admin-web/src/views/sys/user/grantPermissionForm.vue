<template>
	<xn-form-container title="授权权限" :width="drawerWidth" :visible="visible" :destroy-on-close="true"
		:show-pagination="false" @close="onClose">
		<a-alert message="注：此功能界面需要与代码Search条件配合使用，并非所有接口都需设置数据范围，多用于业务模块！" type="warning" closable />
		<a-spin :spinning="spinningLoading">
			<a-table class="mt-4" size="middle" :columns="columns" :data-source="tableLoadData" bordered
				:row-key="(record) => record.api" :pagination="pagination" @change="handleTableChange">
				...
				<template #customFilterDropdown="{ setSelectedKeys, selectedKeys, clearFilters, column }">
					<div class="xn-pd8">
						<a-input ref="searchInput" placeholder="请输入接口或接口名称" :value="selectedKeys[0]" class="xn-jk-line"
							@change="(e) => setSelectedKeys(e.target.value ? [e.target.value] : [])"
							@pressEnter="handleSearch(selectedKeys, column.dataIndex)" />
						<a-button type="primary" size="small" class="xn-wd90 xn-mr8"
						...
						<a-button size="small" class="xn-wd90" @click="handleReset(clearFilters)"> Reset </a-button>
					</div>
				</template>
					...
					<template v-if="column.dataIndex === 'dataScope'">
						<template v-if="record.dataScope.length > 0">
							<template v-for="item in record.dataScope" :key="item.id + record.api">
								<a-radio v-model:checked="item.check" :name="item.title"
									...
									" :count="item.scopeDefineOrgIdList.length" :number-style="{ backgroundColor: '#52c41a' }">
										{{ item.title }}</a-badge>
									<div v-else>{{ item.title }}</div>
								</a-radio>
							</template>
							<a-button v-if="record.dataScope[4].check" type="link" size="small"
								@click="handleDefineOrg(record)">选择组织</a-button>
						</template>
					</template>
				</template>
			</a-table>
