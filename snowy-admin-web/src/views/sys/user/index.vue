<template>
		...
		<a-col :xs="24" :sm="24" :md="24" :lg="19" :xl="19">
			...
			<a-card :bordered="false">
				<s-table ref="tableRef" :columns="columns" :data="loadData" :expand-row-by-click="true" bordered
					:alert="options.alert.show" :row-key="(record) => record.id" :row-selection="options.rowSelection">
					<template #operator class="table-operator">
						<a-space>
							...
							<xn-batch-button :buttonName="$t('common.batchRemoveButton')" icon="DeleteOutlined" buttonDanger
								:selectedRowKeys="selectedRowKeys" @batchCallBack="deleteBatchUser" />
						</a-space>
					</template>
						...
						<template v-if="column.dataIndex === 'action'">
							...
							<a-dropdown>
								...
								<template #overlay>
									<a-menu>
										<a-menu-item>
											<a-popconfirm :title="$t('user.popconfirmResatUserPwd')" placement="topRight"
	...
	<xn-role-selector ref="RoleSelectorPlusRef" :org-tree-api="selectorApiFunction.orgTreeApi"
		:role-page-api="selectorApiFunction.rolePageApi" :add-show="false" :show="false" :role-global="true"
		@onBack="roleBack" />
...
const exportBatchUserVerify = () => {
	if ((selectedRowKeys.value.length < 1) & !searchFormState.value.searchKey & !searchFormState.value.userStatus) {
		message.warning('请输入Search条件或勾选要导出的信息')
	}
