<template>
	<xn-form-container :title="formData.id ? 'Edit移动端菜单' : '增加移动端菜单'" :width="700" :visible="visible"
			...
			<a-row :gutter="16">
				...
				<a-col :span="12">
					<a-form-item label="菜单类型：" name="menuType">
						<a-radio-group v-model:value="formData.menuType" button-style="solid" :options="menuTypeOptions"
							option-type="button" />
					</a-form-item>
				</a-col>
...
const parentChange = (value) => {
	if (value > 0) {
		// 执行接口去Search选择的上级是哪个类别，吧对应的也置为一样的
		const param = {
			id: value
		}
