<template>
		...
		<a-form ref="searchFormRef" name="advanced_search" :model="searchFormState" class="ant-advanced-search-form">
			<a-space style="align-items: normal">
				<a-radio-group v-model:value="module" button-style="solid">
					<a-radio-button v-for="module in moduleList" :key="module.id" :value="module.id"
				...
				<a-form-item name="searchKey">
					<a-space>
						...
						<a-button type="primary" @click="tableRef.refresh(true)">Search</a-button>
						<a-button class="xn-mg08" @click="reset">Reset</a-button>
					</a-space>
				</a-form-item>
			</a-space>
		</a-form>
	...
	<a-card :bordered="false">
		<s-table ref="tableRef" :columns="columns" :data="loadData" :alert="options.alert.show" bordered
			:row-key="(record) => record.id" :tool-config="toolConfig" :show-pagination="false"
			...
			<template #operator class="table-operator">
				<a-space>
					...
					<xn-batch-button buttonName="Batch Delete" icon="DeleteOutlined" buttonDanger
						:selectedRowKeys="selectedRowKeys" @batchCallBack="deleteBatchMobileMenu" />
				</a-space>
			</template>
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						<a @click="formRef.onOpen(record, module)">Edit</a>
