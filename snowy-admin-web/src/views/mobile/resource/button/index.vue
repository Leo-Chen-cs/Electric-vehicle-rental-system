<template>
	<xn-form-container title="按钮权限" :width="700" :visible="visible" :destroy-on-close="true"
		:body-style="{ paddingBottom: '80px' }" @close="onClose">
		<s-table ref="tableRef" :columns="columns" :data="loadData" :alert="false" :row-key="(record) => record.id"
			...
			<template #bodyCell="{ column, record }">
				<template v-if="column.key === 'action'">
					<a @click="buttonForm.onOpen(recordData, record)">Edit</a>
