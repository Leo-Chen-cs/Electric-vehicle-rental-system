<template>
	<a-card :title="title" :bordered="false" class="mt-2" :loading="apiLoading">
		...
		<a-card :bordered="false">
			...
			<div class="add-schedule" @click="addSchedule"><plus-circle-two-tone /> New Schedule</div>
		</a-card>
		<xn-form-container title="New Schedule" :width="700" :visible="visible" :destroy-on-close="true" @close="onClose">
			<a-form ref="formRef" :model="formData" :rules="formRules" layout="vertical">
				<a-form-item label="Time：" name="scheduleTime">
					<a-time-picker v-model:value="scheduleTime" value-format="HH:mm:ss" />
				</a-form-item>
				<a-form-item label="Schedule Desc：" name="scheduleContent">
					<a-textarea v-model:value="formData.scheduleContent" placeholder="请输入日程描述" :maxlength="20"
						:auto-size="{ minRows: 3, maxRows: 3 }" />
				</a-form-item>
			</a-form>
			<template #footer>
				<a-button class="xn-mr8" @click="onClose">Close</a-button>
...
const formRules = {
	...
	scheduleContent: [required('Please Input Content')]
}
